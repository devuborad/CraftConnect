...
export const MarketplacePage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 ios-fade-up">
      ...
      <div className="glass-card bg-white p-4 rounded-2xl border border-stone-200/90 shadow-sm flex flex-col md:flex-row items-center gap-4 hover:shadow-md transition-all">
        ...
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full bg-stone-50 border border-stone-200 rounded-xl pl-10 pr-4 py-2.5 text-xs text-stone-900 focus:outline-none focus:ring-2 focus:ring-[#C85A32] focus:bg-white transition-all"
          />
        </div>
        ...
        <div className="flex items-center space-x-2 w-full md:w-auto">
          ...
          <select
            ...
            className="bg-stone-50 border border-stone-200 rounded-xl px-3 py-2.5 text-xs font-semibold text-stone-800 focus:outline-none focus:ring-2 focus:ring-[#C85A32] transition-all cursor-pointer"
          >
            {locations.map((loc) => (
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all active:scale-95 ${
              selectedCategory === cat
                ? 'bg-[#4A2E1B] text-white shadow-md scale-[1.02]'
      ...
      {filteredProducts.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredProducts.map((product, idx) => (
            <div 
              key={product.id} 
              className={`ios-fade-up ${idx < 4 ? `ios-delay-${idx + 1}` : ''}`}
            >
              <ProductCard
                product={product}
