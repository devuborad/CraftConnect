...
import { ArrowRight, Upload, Share2, ArrowLeft, Sparkles, FolderOpen, X, Trash2 } from 'lucide-react';
...
export const AddProductWizardPage: React.FC = () => {
  ...
  const handleTranscriptComplete = async (text: string, langName: string = 'Gujarati') => {
    ...
    try {
      const res = await aiService.generateCatalogue(text, langCode);
      setCatalogue(res);
      showToast('Catalogue generated by AI ✨', 'Review title, material, and descriptions', 'success');
    } catch {
      showToast('AI Generation failed', 'Loaded template catalogue info', 'warning');
      setCatalogue({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          <Link
            ...
            className="bg-white hover:bg-stone-50 text-stone-800 border border-stone-300 px-4 py-2 rounded-2xl text-xs font-bold transition-all flex items-center space-x-2 shadow-xs hover:border-[#C85A32]"
          >
        ...
        <div className="flex items-center space-x-2 self-start sm:self-auto">
          <button
            ...
            className="bg-purple-50 hover:bg-purple-100 text-purple-800 border border-purple-200 px-3.5 py-2 rounded-2xl text-xs font-bold transition-all flex items-center space-x-1.5 shadow-xs cursor-pointer"
          >
          ...
          <button
            ...
            className="bg-amber-100 hover:bg-amber-200 text-[#C85A32] border border-amber-300 px-3.5 py-2 rounded-2xl text-xs font-bold transition-all cursor-pointer shadow-xs"
          >
            📁 Save Draft
          </button>
        </div>
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center relative z-10 transition-all ${
                  step.num < currentStep ? 'cursor-pointer hover:scale-110' : 'cursor-default'
                }`}
      ...
      {currentStep > 1 && currentStep <= 5 && (
        <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-stone-200 shadow-xs animate-in fade-in duration-200">
          <button
            type="button"
            onClick={() => setCurrentStep((prev) => Math.max(1, prev - 1))}
            className="bg-stone-100 hover:bg-stone-200 text-stone-800 border border-stone-300 px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center space-x-2 cursor-pointer shadow-xs hover:border-[#C85A32]"
          >
            <ArrowLeft className="w-4 h-4 text-[#C85A32]" />
        ...
        <div className="bg-white rounded-3xl p-6 border border-stone-200 shadow-md space-y-6 animate-in fade-in duration-200">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
            <button
              type="button"
              onClick={handleSaveDraft}
              className="w-full sm:w-1/2 bg-stone-100 hover:bg-stone-200 text-stone-800 border border-stone-300 py-3.5 rounded-xl font-bold text-xs flex items-center justify-center space-x-2 shadow-sm transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full sm:w-1/2 bg-[#C85A32] hover:bg-[#b04b27] text-white py-3.5 rounded-xl font-bold text-xs shadow-xl flex items-center justify-center space-x-2 transition-all cursor-pointer"
            >
      ...
      {currentStep === 6 && (
        <div className="bg-white rounded-3xl p-10 border border-stone-200 shadow-xl text-center space-y-6 animate-in zoom-in-95 duration-300">
          ...
          {publishedProduct && (
            <div className="glass-card bg-[#FAF7F2] p-6 rounded-2xl border border-amber-200 max-w-sm mx-auto flex items-center space-x-4 text-left">
              <img
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3 pt-4">
            <Link
              to="/marketplace"
              className="bg-[#C85A32] hover:bg-[#b04b27] text-white px-6 py-3 rounded-2xl font-bold text-xs shadow-lg transition-all"
            >
              View in Marketplace 🛍️
            </Link>
            <button
              onClick={() => {
                setPhotoUrl('');
              ...
              className="bg-amber-100 hover:bg-amber-200 text-[#C85A32] px-6 py-3 rounded-2xl font-bold text-xs transition-all cursor-pointer"
            >
              + Add Another Product ✨
            </button>
            <Link
              to="/artisan/dashboard"
              className="bg-stone-100 hover:bg-stone-200 text-stone-800 px-6 py-3 rounded-2xl font-bold text-xs transition-all"
            >
              Back to Dashboard 📊
            </Link>
          </div>
