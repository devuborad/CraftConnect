      ...
      <body>
        ...
        <div class="kpi-grid">
          <div class="kpi-card">
            ...
            <div class="kpi-value">₹${stats.grandTotalValue.toLocaleString('en-IN')}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Realized Liquid Cash</div>
            <div class="kpi-value" style="color:#047857;">₹${stats.realizedRevenue.toLocaleString('en-IN')}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Bulk Wholesale Value</div>
            <div class="kpi-value" style="color:#C85A32;">₹${stats.bulkTotalValue.toLocaleString('en-IN')}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Direct Retail Value</div>
            <div class="kpi-value" style="color:#065f46;">₹${stats.directTotalValue.toLocaleString('en-IN')}</div>
          </div>
        </div>

        <div class="section-title">1. Bulk Wholesale Queries vs Direct Retail Summary</div>
        <table>
          <thead>
            <tr>
              <th>Channel Type</th>
              <th>Deals Count</th>
              <th>Total Volume (Units)</th>
              <th>Total Value (₹)</th>
          ...
          <tbody>
            <tr>
              <td class="highlight-bulk">Bulk Wholesale Queries 📦</td>
              <td>${stats.bulkList.length} Inquiries</td>
              <td>${stats.bulkTotalUnits} units</td>
              <td class="highlight-bulk">₹${stats.bulkTotalValue.toLocaleString('en-IN')}</td>
              <td>₹${stats.bulkAOV.toLocaleString('en-IN')}</td>
              <td>₹${stats.bulkAvgUnitPrice} / unit</td>
            </tr>
            <tr>
              <td class="highlight-direct">Direct Retail Orders ⚡</td>
              <td>${stats.directList.length} Orders</td>
              <td>${stats.directTotalUnits} units</td>
              <td class="highlight-direct">₹${stats.directTotalValue.toLocaleString('en-IN')}</td>
              <td>₹${stats.directAOV.toLocaleString('en-IN')}</td>
              <td>₹${stats.directAvgUnitPrice} / unit</td>
            </tr>
          </tbody>
        </table>

        <div class="section-title">2. Key Channel Differential Metrics</div>
        <table>
          <thead>
            <tr>
              <th>Differential Parameter</th>
              <th>Bulk Wholesale Channel</th>
              <th>Direct Retail Channel</th>
              <th>Channel Strategic Insight</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td><strong>Price Realization / Unit</strong></td>
              <td>₹${stats.bulkAvgUnitPrice} / unit (Wholesale Batch)</td>
              <td>₹${stats.directAvgUnitPrice} / unit (Full Retail Price)</td>
              <td>Direct Retail yields higher profit margin per craft item.</td>
            </tr>
            <tr>
              <td><strong>Order Volume Scale</strong></td>
              <td>${stats.bulkTotalUnits} units total (${stats.bulkList.length > 0 ? Math.round(stats.bulkTotalUnits / stats.bulkList.length) : 0} units/deal)</td>
              <td>${stats.directTotalUnits} units total (${stats.directList.length > 0 ? (stats.directTotalUnits / stats.directList.length).toFixed(1) : 0} units/order)</td>
              <td>Bulk queries provide high production volume utilization.</td>
            </tr>
            <tr>
              <td><strong>Cash Flow Liquidity</strong></td>
              <td>Milestone / Invoice Deposit Terms</td>
              <td>100% Upfront Digital Gateway Pre-payment</td>
              <td>Direct sales deliver immediate liquid cashflow.</td>
            </tr>
          </tbody>
        </table>

        <div class="footer">
          <div>CraftConnect Official Platform Report &bull; Confidential Artisan Audit</div>
          <div>Page 1 of 1</div>
        </div>
